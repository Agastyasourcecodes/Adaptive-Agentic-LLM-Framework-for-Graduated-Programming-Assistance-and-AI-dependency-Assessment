 ...
 *
 * Providers:
 *   qwen    Local Ollama (always the fallback target)
 *   gemini  Google Gemini API  (GEMINI_API_KEY) - DEFAULT
 *   groq    Groq API           (GROQ_API_KEY)
 *
 * The router never throws for a cloud-provider problem — it falls back to
 * Qwen and reports what happened so the UI / trace can show it.
 * It only throws if Qwen itself is unreachable.
 */
...
/* ---------- cool-down after quota exhaustion ---------- */
...
async function classifyHttpError(res) {
  ...
  if (
    res.status === 429 ||
    res.status === 402 ||
    quotaWords.some((w) => lower.includes(w))
  ) {
    return new ProviderError(
      "quota",
      `Quota/rate limit (${res.status})`,
      res.status
    );
  }
  ...
  return new ProviderError(
    "bad_request",
    `Request rejected (${res.status}): ${body.slice(0, 160)}`,
    res.status
  );
}
  ...
  gemini: {
    ...
    async call(prompt) {
      ...
      if (!text) {
        throw new ProviderError(
          "empty",
          "Gemini returned no text (possibly blocked)"
        );
      }
 ...
 *
 * NOTE:
 * Although the comment in the original code said Gemini was default,
 * the actual value was "qwen".
 *
 * Keeping Qwen as default here preserves the original behavior.
 */
...
/**
 * Public status of all providers.
...
export async function generate(
  prompt,
  requestedId = DEFAULT_PROVIDER
) {
  const requested = PROVIDERS[requestedId]
    ? requestedId
    : DEFAULT_PROVIDER;
  ...
  if (requested !== "qwen") {
    ...
    try {
      ...
      if (remainingCooldown(requested) > 0) {
        throw new ProviderError(
          "cooldown",
          "cooling down"
        );
      ...
      /*
       * Put provider into cooldown after quota/auth
       * failures to avoid repeatedly hitting the API.
       */
      if (
        kind === "quota" ||
        kind === "auth"
      ...
      console.warn(
        `[llmRouter] ${requested} failed (${kind}) → falling back to Qwen. ${err.message}`
      );
    }
  }
  ...
  try {
    ...
    const e = new Error(
      `Qwen (Ollama) is unreachable${
        fallbackReason
          ? ` and ${fallbackReason}`
          : ""
      }. Is "ollama serve" running with the configured model?`
    );
